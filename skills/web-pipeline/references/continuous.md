...
- 작은 연결 기능은 하나의 task로 처리할 수 있다. 의도적으로 분할한 멤버가 프론트·백엔드 양쪽의 구현을 필요로 할 때는 `implementation_groups`를 쓴다(`Docs/Runbooks/IMPLEMENTATION_GROUPS.md`): 계약·범위 정의 → 모든 멤버와 Phase `prepare` → 모든 멤버 Baseline·진입 게이트 → 그룹 `order`대로 백엔드, 프론트 구현 → 멤버별 Fast/Task/리뷰 → Phase 검증·리뷰. 그룹 내부의 `depends_on`은 거부된다.
...
| `WAITING` | 안전하게 진행할 작업이 없다 | 원인 조사 후 기존 동의 전사·범위 안의 수리·복구를 수행하고 retry/next. 실제 미해결 결정·외부 차단만 보고한다. 변화 없이 next를 반복하지 않는다 |
| `PAUSED_LIMIT` | 큐 또는 작업 예산 소진 | 두 한도를 모두 보고하고 **멈춘다**. 아래 "예산" 참고 |
| `BUSY` | 끝나지 않은 액션이 큐를 점유 | 두 번째 워커를 돌리지 않는다. 아래 "복구" 참고 |
| Git 통합 미완료 | 머지·리베이스 등이 진행 중이거나 실패 | 새 기능 작업을 보류하고 기존 요청 범위에서 변경을 보존하며 충돌을 해결한다. 내용 선택이 불명확하거나 변경 폐기가 필요할 때만 결정을 구한다. 실패를 건너뛰거나 큐를 새로 만들지 않는다 |
...
새 도입은 누적 시간 `time_budget_mode: warn`이므로 시간 초과만으로 멈추거나 renew하지 않는다. 개별 명령 timeout은 유지한다. 기존 설정/큐에서 키가 없으면 `enforce`를 유지한다. 새 큐는 생성 시 프로젝트 기본값을 고정한다.
