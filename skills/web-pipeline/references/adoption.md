...
3. **완결된 기능 하나 검증.** 요청 범위 안에서 실제 경계를 가로지르는 작은 기능을 고른다. UI 상호작용·API 응답·영속화·오류 경로의 관찰 가능한 수용 기준을 정한다. 작은 연결 기능은 프론트·백엔드를 포함한 작업 하나로 완료할 수 있다. 여러 멤버로 이미 분할했고 서로의 구현이 있어야 검증 가능한 경우에만 Phase + `implementation_groups`를 쓴다([continuous.md](continuous.md)). mock만으로는 연결이 증명되지 않는다. 빌드 성공·doctor 통과·설정 검증은 기능 완료가 아니다.
...
2. 사용자가 업데이트 적용을 이미 요청했다면 중복 확인 없이 `... upgrade --target <경로> --apply`. 미리보기만 요청했다면 적용하지 않는다. 관리 대상 엔진 바이트를 백업하고 교체하며, 반환된 `transaction` ID를 보고한다.
...
## 설정이 제품 개발을 대신하지 않게 하기

[product-first.md](product-first.md)를 따른다. 6개 source 역할에 기존 문서를 연결하고, 첫 기능에 필요한 실제 검사부터 연결한다. 준비 조건이 갖춰지면 파이프라인 확장을 멈추고 제품 구현으로 이동한다. `--domains`를 생략한 전체 예시는 해야 할 기능 목록이 아니다.
