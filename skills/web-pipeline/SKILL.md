...
5. `approval_policy: standard`에서 보호되지 않은 T0–T2는 사용자 승인 게이트가 없다. 요청 범위 안의 구현·수정·검증·로컬 리뷰는 단계마다 허락을 구하지 말고 계속한다. T1/T2 리뷰는 `pipeline-reviewer` 서브에이전트로 새 컨텍스트에서 수행하고 기록한다. 이는 사람의 승인이 아니다.
...
8. 예산 한도에 걸리면 멈추고 보고한다. 스스로 `loop renew`하지 않는다.
