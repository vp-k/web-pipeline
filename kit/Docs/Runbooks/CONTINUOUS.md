...
Time-budget exhaustion is a non-PASS result and spends one failed attempt, but not an external retry. Budget-only interruption preserves the preceding same-failure guard rather than replacing it with a timeout fingerprint. Genuine execution blockers and command-specific timeouts still count normally. Queue limits are checked between actions; in-flight verification has its own task budget/command timeouts. An agent must return/complete its lease; the CLI cannot kill a host model.
...
2. Obtain the user's request to resume with a bounded additional budget. Record that reason via `loop renew --queue` or `loop renew --task ID`. They are exclusive targets; if both limits are exhausted, renew each separately. No trust file or human name is needed. Each grant accepts 0–1440 extra minutes and 0–1000 extra attempts, with at least one positive; queue extra-attempts means additional steps.
