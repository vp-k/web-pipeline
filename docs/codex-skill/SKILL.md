...
- READY scope/AC/DoD changes require `revise` and renewed evidence/approvals. Usage and renewal history remain cumulative. On limits, stop and report them; a new user request to resume may authorize one bounded `loop renew --queue` or `--task ...` grant. Read continuous guidance, record the request/reason and added budget, then resume through existing gates. Never self-renew repeatedly under an earlier generic keep-going instruction.
